...
async def on_ready(): # on_ready() event : when the bot has finised logging in and setting things up
    await client.change_presence(status=discord.Status.online, activity=discord.Game("/도움말 or /help"))
...
async def on_message(message): # on_message() event : when the bot has recieved a message
    ...
    if message.content.startswith("/help") or message.content.startswith("/도움말"):
        embed = discord.Embed(title="명령어 사용방법", description="/롤전적 (소환사 이름 - 띄어쓰기 붙여쓰기 상관 X)", color=0x5CD1E5)
        embed.set_footer(text='Service provided by 우푸봇.',
                         icon_url='https://secure.gravatar.com/avatar/154f1c4a5f6b1c41db10dd2ba88cef46?size=35')
